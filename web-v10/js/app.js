...
(function () {
  ...
  const state = {
    ...
    loteUnif: null,
    // v10 — volumétrico por cara (Fase 1). Opt-in: si está apagado, el volumétrico se comporta como hasta v9.
    volPorCara: false, volCaraSel: "tapa", volCaras: {}, volAristas: {},
  };
  let piezaSeq = 0;
  const BORDE_DEFAULTS = { borde: 0.045, unionCierre: 0.045 };

  // ---------- v10: modelo de caras del volumétrico ----------
  // 5 caras de la caja desplegada (cruz). Cada cara es un paño plano cuyas dimensiones
  // se DERIVAN de ancho (A), largo (L) y alto (H). Editar una cara reusa las mismas
  // herramientas del paño (ojetillos/bordes/straps/aletas), cada feature anclado a 1 cara.
  // Tapa = A×L; Frente/Fondo = A×H (alas a lo ancho); Izquierda/Derecha = L×H (alas a lo largo).
  const VOL_CARAS = [
    { id: "tapa", nombre: "Tapa", dim: (A, L, H) => ({ w: A, h: L }) },
    { id: "frente", nombre: "Frente", dim: (A, L, H) => ({ w: A, h: H }) },
    { id: "posterior", nombre: "Posterior", dim: (A, L, H) => ({ w: A, h: H }) },
    { id: "izq", nombre: "Izquierda", dim: (A, L, H) => ({ w: L, h: H }) },
    { id: "der", nombre: "Derecha", dim: (A, L, H) => ({ w: L, h: H }) },
  ];
  // Aristas compartidas (pliegue/costura). En Fase 1 las 4 que rodean la tapa.
  const VOL_ARISTAS = [
    { id: "tapa-frente", nombre: "Tapa ↔ Frente" },
    { id: "tapa-posterior", nombre: "Tapa ↔ Posterior" },
    { id: "tapa-izq", nombre: "Tapa ↔ Izquierda" },
    { id: "tapa-der", nombre: "Tapa ↔ Derecha" },
  ];
  // Contenedor de features de una cara (mismo esquema que usan los editores de paño).
  function volCaraVacia() {
    return {
      ojMode: "total", ojTotal: 0, ojEdges: null, ojParejo: false,
      bordeModo: "uniforme", bordeValor: "0.045", bordeRotUnif: false,
      bordes: {
        sup: { tipo: "borde", valor: "0.045", diam: "" },
        inf: { tipo: "borde", valor: "0.045", diam: "" },
        izq: { tipo: "borde", valor: "0.045", diam: "" },
  ...
  // Garantiza que existan los contenedores de cada cara y arista (no pisa lo ya cargado).
  function ensureVolCaras() {
    if (!state.volCaras) state.volCaras = {};
    if (!state.volAristas) state.volAristas = {};
    VOL_CARAS.forEach((c) => { if (!state.volCaras[c.id]) state.volCaras[c.id] = volCaraVacia(); });
    VOL_ARISTAS.forEach((a) => { if (!state.volAristas[a.id]) state.volAristas[a.id] = { tipo: "pliegue" }; });
  }
  // Dimensiones derivadas de cada cara para las medidas actuales (A,L,H). Devuelve [] si falta H.
  function volCarasDims() {
    const A = num("f_ancho", 0), L = num("f_largo", 0), H = alturaUnif();
    if (!(A > 0) || !(L > 0) || !(H > 0)) return [];
    return VOL_CARAS.map((c) => { const d = c.dim(A, L, H); return { id: c.id, nombre: c.nombre, w: d.w, h: d.h }; });
  }
  // ¿La cara tiene algún feature ingresado? (para marcar "genera plano").
  function volCaraConDatos(cara) {
    if (!cara) return false;
    if ((cara.straps || []).length || (cara.aletas || []).length) return true;
    if (cara.ojMode === "arista" && cara.ojEdges) {
      if (["sup", "inf", "izq", "der"].some((k) => { const e = cara.ojEdges[k]; return e && ((e.sets || []).length || (parseFloat(e.dist) || 0) > 0 || (parseFloat(e.total) || 0) > 0); })) return true;
    } else if ((parseFloat(cara.ojTotal) || 0) > 0) return true;
    if (cara.bordeModo === "arista") {
      if (["sup", "inf", "izq", "der"].some((k) => { const b = cara.bordes && cara.bordes[k]; return b && b.tipo && b.tipo !== "bruto"; })) return true;
    }
    return false;
  }
  // VC6 — Spec de dibujo de una cara: reusa sketchPieza armando un objeto tipo-pieza con las
  // medidas derivadas de la cara y sus features (ojetillos/bordes/straps/aletas). Sin alto (plano).
  function volCaraSpec(caraId) {
    const cara = state.volCaras[caraId]; if (!cara) return null;
    const dim = volCarasDims().find((d) => d.id === caraId) || { w: 0, h: 0 };
    const pzLike = {
      ancho: String(dim.w), largo: String(dim.h), inscritos: [], cortes: [],
      bordeModo: cara.bordeModo, bordeValor: cara.bordeValor, bordeRotUnif: cara.bordeRotUnif, bordes: cara.bordes,
      unionRot: cara.unionRot, union: cara.union || "0.045", orient: "largo",
      telaNombre: ($("f_tela") ? $("f_tela").value : ""),
      ojMode: cara.ojMode || "arista", ojEdges: cara.ojEdges || {}, ojParejo: cara.ojParejo, ojetillos: cara.ojTotal || 0,
      straps: cara.straps, aletas: cara.aletas, usaAlto: false,
    };
    return sketchPieza(pzLike);
  }
  // VC5 — Despliegue en cruz dibujado desde las medidas actuales. Resalta la cara seleccionada,
  // marca con punto verde las caras con features (generan plano) y dibuja cada arista según su
  // tipo (pliegue = punteado verde · costura = línea sólida).
  function volCruzSVG(sel) {
    const A = num("f_ancho", 0), L = num("f_largo", 0), H = alturaUnif();
    if (!(A > 0) || !(L > 0) || !(H > 0)) return '<p class="muted small">Ingresa largo, ancho y alto para ver el despliegue.</p>';
    const Wd = A + 2 * H, Ld = L + 2 * H;
    const sc = Math.min(300 / Wd, 230 / Ld), ox = 10, oy = 10;
    const f = (n) => (+n).toFixed(1), fm = window.CalcCIBSA.fmtNum;
    const X = (x) => ox + x * sc, Y = (y) => oy + y * sc;
    const totalW = ox * 2 + Wd * sc, totalH = oy * 2 + Ld * sc;
    const NOM = { tapa: "Tapa", frente: "Frente", posterior: "Posterior", izq: "Izq.", der: "Der." };
    // En la cruz: izq/der se dibujan H de ancho × L de alto, pero su medida real es L×H.
    const faces = [
      { id: "posterior", mx: H, my: 0, dw: A, dh: H, rw: A, rh: H },
      { id: "frente", mx: H, my: H + L, dw: A, dh: H, rw: A, rh: H },
      { id: "izq", mx: 0, my: H, dw: H, dh: L, rw: L, rh: H },
      { id: "der", mx: H + A, my: H, dw: H, dh: L, rw: L, rh: H },
      { id: "tapa", mx: H, my: H, dw: A, dh: L, rw: A, rh: L },
    ];
    let s = '<svg viewBox="0 0 ' + f(totalW) + " " + f(totalH) + '" width="100%" xmlns="http://www.w3.org/2000/svg">';
    faces.forEach((fc) => {
      const conD = volCaraConDatos(state.volCaras[fc.id]);
      const isSel = fc.id === sel;
      const x = X(fc.mx), y = Y(fc.my), w = fc.dw * sc, h = fc.dh * sc;
      const fill = fc.id === "tapa" ? "var(--field)" : "var(--surface)";
      const stroke = isSel ? "var(--accent)" : "var(--border)", sw = isSel ? 2.4 : 1.2;
      s += '<rect x="' + f(x) + '" y="' + f(y) + '" width="' + f(w) + '" height="' + f(h) + '" rx="2" fill="' + fill + '" stroke="' + stroke + '" stroke-width="' + sw + '"/>';
      if (conD) s += '<circle cx="' + f(x + w - 6) + '" cy="' + f(y + 6) + '" r="3" fill="#1B7f5f"/>';
      s += '<text x="' + f(x + w / 2) + '" y="' + f(y + h / 2 - 1) + '" text-anchor="middle" font-size="9.5" fill="var(--text)">' + NOM[fc.id] + "</text>";
      s += '<text x="' + f(x + w / 2) + '" y="' + f(y + h / 2 + 9) + '" text-anchor="middle" font-size="8" fill="var(--muted)">' + fm(fc.rw) + "×" + fm(fc.rh) + "</text>";
    });
    const seg = (id, x1, y1, x2, y2) => {
      const t = (state.volAristas[id] && state.volAristas[id].tipo) || "pliegue";
      const dash = t === "pliegue" ? ' stroke-dasharray="5 4"' : "", col = t === "pliegue" ? "#1D9E75" : "#9a6a2c";
      return '<line x1="' + f(x1) + '" y1="' + f(y1) + '" x2="' + f(x2) + '" y2="' + f(y2) + '" stroke="' + col + '" stroke-width="2"' + dash + "/>";
    };
    s += seg("tapa-posterior", X(H), Y(H), X(H + A), Y(H));
    s += seg("tapa-frente", X(H), Y(H + L), X(H + A), Y(H + L));
    s += seg("tapa-izq", X(H), Y(H), X(H), Y(H + L));
    s += seg("tapa-der", X(H + A), Y(H), X(H + A), Y(H + L));
    s += "</svg>";
    return s;
  }
  // VC2 — Panel del volumétrico: interruptor "editar por cara" + selector de caras + despliegue.
  function renderVolCarasPanel(container) {
    if (!container) return;
    ensureVolCaras();
    const panel = document.createElement("div"); panel.className = "volcaras-panel";
    const tog = document.createElement("label"); tog.className = "chk volcaras-toggle";
    const cb = document.createElement("input"); cb.type = "checkbox"; cb.checked = !!state.volPorCara;
    const sp = document.createElement("span"); sp.textContent = "Editar elementos por cara (volumétrico)";
    cb.addEventListener("change", () => { state.volPorCara = cb.checked; recompute(); });
    tog.appendChild(cb); tog.appendChild(sp); panel.appendChild(tog);
    if (state.volPorCara) {
      const dims = volCarasDims();
      const chips = document.createElement("div"); chips.className = "volcara-chips";
      VOL_CARAS.forEach((c) => {
        const d = dims.find((x) => x.id === c.id);
        const conD = volCaraConDatos(state.volCaras[c.id]);
        const b = document.createElement("button"); b.type = "button";
        b.className = "volcara-chip" + (state.volCaraSel === c.id ? " sel" : "") + (conD ? " con-datos" : "");
        b.innerHTML = '<span class="vc-nom">' + esc(c.nombre) + "</span>" +
          (d ? '<span class="vc-dim">' + window.CalcCIBSA.fmtNum(d.w) + "×" + window.CalcCIBSA.fmtNum(d.h) + " m</span>" : "") +
          (conD ? '<span class="vc-dot" title="Tiene features: genera plano">●</span>' : "");
        b.addEventListener("click", () => { state.volCaraSel = c.id; recompute(); });
        chips.appendChild(b);
      });
      panel.appendChild(chips);
      const ov = document.createElement("div"); ov.className = "volcara-cruz"; ov.innerHTML = volCruzSVG(state.volCaraSel);
      panel.appendChild(ov);
      // VC4 — Tipo de cada arista compartida (manual): pliegue (tela continua) o costura (paneles unidos).
      const arWrap = document.createElement("div"); arWrap.className = "volcara-aristas";
      const arCap = document.createElement("div"); arCap.className = "muted small"; arCap.textContent = "Tipo de cada arista (unión tapa ↔ pared):";
      arWrap.appendChild(arCap);
      VOL_ARISTAS.forEach((a) => {
        if (!state.volAristas[a.id]) state.volAristas[a.id] = { tipo: "pliegue" };
        const row = document.createElement("div"); row.className = "volcara-arista-row";
        const lbl = document.createElement("span"); lbl.className = "va-nom"; lbl.textContent = a.nombre;
        const sel = document.createElement("select"); sel.className = "va-sel";
        [["pliegue", "Pliegue (tela continua)"], ["costura", "Costura (paneles unidos)"]].forEach(([v, t]) => { const o = document.createElement("option"); o.value = v; o.textContent = t; sel.appendChild(o); });
        sel.value = state.volAristas[a.id].tipo || "pliegue";
        sel.addEventListener("change", (e) => { state.volAristas[a.id].tipo = e.target.value; recompute(); });
        row.appendChild(lbl); row.appendChild(sel); arWrap.appendChild(row);
      });
      panel.appendChild(arWrap);
      const note = document.createElement("p"); note.className = "muted small";
      note.innerHTML = "Selecciona una cara para editar sus elementos. <b style='color:#1B7f5f'>●</b> = la cara tiene features y generará su plano. Aristas: <span style='color:#1D9E75'>‑ ‑ pliegue</span> · <span style='color:#9a6a2c'>— costura</span>.";
      panel.appendChild(note);
      // VC6 — Planos por cara (solo las que tienen features), dibujados desde su spec.
      const conPlano = VOL_CARAS.map((c) => c.id).filter((id) => volCaraConDatos(state.volCaras[id]));
      if (conPlano.length && window.SketchCIBSA) {
        const pc = document.createElement("div"); pc.className = "volcara-planos";
        const h = document.createElement("div"); h.className = "volcara-sub-h"; h.textContent = "Planos por cara (solo las que tienen features):"; pc.appendChild(h);
        conPlano.forEach((id) => {
          const meta = VOL_CARAS.find((c) => c.id === id);
          const card = document.createElement("div"); card.className = "volcara-plano";
          const t = document.createElement("div"); t.className = "volcara-plano-h"; t.textContent = meta ? meta.nombre : id; card.appendChild(t);
          const box = document.createElement("div"); box.className = "sketch";
          try { box.innerHTML = window.SketchCIBSA.sketchSVG(volCaraSpec(id), { live: true }); } catch (e) { box.innerHTML = '<p class="muted small">No se pudo dibujar.</p>'; }
          card.appendChild(box); pc.appendChild(card);
        });
  ...
  // VC3 — Editor por cara: monta los editores existentes (ojetillos/bordes/straps/aletas)
  // apuntando a la cara activa, en un contenedor estable (#wVolCaraEditor) para no perder el foco
  // al escribir (recompute reconstruye el panel del plano, pero NO este editor).
  let _volEditorKey = null;
  function volSubBloque(host, titulo) {
    const wrap = document.createElement("div"); wrap.className = "volcara-sub";
    const h = document.createElement("h4"); h.className = "volcara-sub-h"; h.textContent = titulo; wrap.appendChild(h);
    const body = document.createElement("div"); wrap.appendChild(body);
    host.appendChild(wrap); return body;
  }
  function renderVolCaraEditor(host, caraId) {
    host.innerHTML = "";
    const cara = state.volCaras[caraId]; if (!cara) return;
    const meta = VOL_CARAS.find((c) => c.id === caraId);
    const getDim = () => volCarasDims().find((d) => d.id === caraId) || { w: 0, h: 0 };
    const getA = () => getDim().w, getL = () => getDim().h;
    const d0 = getDim();
    const tit = document.createElement("h3"); tit.className = "section";
    tit.innerHTML = "Editar cara: <span style='color:var(--accent)'>" + esc(meta ? meta.nombre : caraId) + "</span> <span class='muted small'>(" + window.CalcCIBSA.fmtNum(d0.w) + "×" + window.CalcCIBSA.fmtNum(d0.h) + " m)</span>";
    host.appendChild(tit);
    cara.ojMode = "arista"; if (!cara.ojEdges) cara.ojEdges = {};
    renderOjetillosArista(volSubBloque(host, "Ojetillos por arista"), cara.ojEdges, cara, getA, getL, () => 0, recompute, () => []);
    renderPiezaBordes(volSubBloque(host, "Bordes y uniones"), cara, recompute);
    renderStraps(volSubBloque(host, "Straps / cintas"), { straps: cara.straps, cantidad: () => 1, getAncho: getA, getLargo: getL, onChange: recompute });
    renderAletas(volSubBloque(host, "Aletas / anexos"), { aletas: cara.aletas, cantidad: () => 1, valorOj: valorOjUnif, factor: facUnif, onChange: recompute });
  }
  function actualizarVolCaraEditor() {
    const host = $("wVolCaraEditor"); if (!host) return;
    const on = state.prodMode === "uniforme" && alturaUnif() > 0 && state.volPorCara;
    host.classList.toggle("hidden", !on);
    if (!on) { _volEditorKey = null; host.innerHTML = ""; return; }
    ensureVolCaras();
    if (state.volCaraSel === _volEditorKey && host.childElementCount) return; // ya montado para esta cara
    _volEditorKey = state.volCaraSel;
    renderVolCaraEditor(host, state.volCaraSel);
  }
  ...
  function renderGranel() {
    ...
    lista.forEach((p) => {
      ...
      if (p.precio == null) {
        ...
        const inp = document.createElement("input"); inp.type = "text"; inp.inputMode = "decimal"; inp.className = "granel-cant"; inp.placeholder = "cant. (" + esc(p.unidad) + ")";
        agregarCalc(inp);
        ...
        const add = () => { const c = window.CalcCIBSA.evalExpr(inp.value); if (c == null || isNaN(c) || c <= 0) { inp.focus(); return; } granelAgregar(p, c); };
  ...
  function nuevaCorte(base) {
    ...
    return {
      ...
      tipo: base ? (base.tipo || "corte") : "corte", fade: base ? (base.fade || "") : "",
  ...
  function recomputeUniforme() {
    ...
    if (sk && window.SketchCIBSA && !document.body.classList.contains("no-plano")) {
      ...
      if (alturaUnif() > 0) renderVolCarasPanel(sk); // VC2/VC5: caras + despliegue (solo volumétrico)
    }
    actualizarVolCaraEditor(); // VC3: editor por cara en contenedor estable
  ...
  function renderPiezaBordes(container, pz, onChangeFn) {
    container.innerHTML = "";
    const onChange = onChangeFn || recomputeCompuesto;
    { const chk = document.createElement("label"); chk.className = "chk borde-rot-chk";
    ...
    selM.addEventListener("change", (e) => { pz.bordeModo = e.target.value; renderPiezaBordes(container, pz, onChangeFn); onChange(); });
    ...
    if ((pz.bordeModo || "uniforme") === "uniforme") {
      ...
      aristas.forEach(([key, label]) => {
        ...
        const warn = document.createElement("div"); warn.className = "oj-err"; warn.style.fontSize = "12px";
        const refWarn = () => { const t = (b.tipo === "borde_cuerda" || b.tipo === "bolsillo") ? avisoDiamGrande(b.diam) : ""; warn.textContent = t; warn.style.display = t ? "" : "none"; };
        sel.addEventListener("change", (e) => { b.tipo = e.target.value; renderPiezaBordes(container, pz, onChangeFn); onChange(); });
