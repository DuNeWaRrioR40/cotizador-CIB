...
(function () {
  ...
  const state = {
    ...
    ojAristas: [], ojEdges: null, ojParejo: false, ojNumerar: false, cotasOcultas: {}, ojError: "", trasUnif: false, ultimoPdf: null, progTimer: null, progVal: 0,
  ...
  const SNAP_STATE = ["orientacionSel", "orientUnif", "ojMode", "ojTotal", "ojSubstate", "ojAristasN", "ojAristas", "ojEdges", "ojParejo", "ojNumerar", "cotasOcultas", "trasUnif", "docMode", "prodMode", "complementosUnif", "cortesUnif", "backCortesUnif", "backComplementosUnif", "aletasUnif", "backAletasUnif", "strapsUnif", "bordeModo", "bordeValor", "bordeRotUnif", "unionRot", "bordes", "piezas", "factorUnif", "granelLineas"];
  ...
  function menuPlano(container, grupos, onToggle, cotasCtl) {
    if (!container) return;
    ...
    if (!filas.length && !tieneCotas) return;
  ...
  function posicionesEdge(k, L, d, parejo, removed, edges, nTot) {
    ...
    if (nTot != null && nTot >= 2) {
      ...
      if (removed && removed.length) {
        // Arista seccionada por un corte: el "número" no se mantiene fijo; se respeta el DISTANCIAMIENTO
        // que ese número implica en la arista completa (d = L/(N-1)) y se redistribuye sobre los tramos
        // que quedan, poniendo ojetillo en cada esquina nueva.
        const dImp = L / (N - 1);
        full = window.SketchCIBSA.posicionesAristaSeg(L, dImp, !!parejo, removed);
      } else {
        // Sin seccionar: N ojetillos repartidos parejo en toda la arista (incluye las 2 esquinas).
