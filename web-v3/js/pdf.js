...
(function (global) {
  ...
  async function generarCotizacionCompuesta(datos) {
    ...
    datos.piezas.forEach((pz) => {
      ...
      const orientTxt = pz.orient === "menor" ? "costuras a lo ancho" : "costuras a lo largo";
      const detail = [[etq || pz.tela.nombre, true]];
      if (etq) detail.push([pz.tela.nombre, false]);
      (pz.tela.ficha || []).forEach((s) => detail.push([s, false]));
      detail.push([`Formato ${pz.largo}×${pz.ancho} m · ${pz.ojetillos} ojetillos c/u · ${orientTxt}`, false]);
      const vu = c.material + c.ojetillosValor;   // neto por 1 unidad (material + ojetillos)
      itemRow(String(pz.cantidad), detail, money(vu), money(c.subtotal));
    });

    // Totales
    const subtotal = datos.piezas.reduce((s, p) => s + p.calc.subtotal, 0);
