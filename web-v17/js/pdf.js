...
(function (global) {
  ...
  function elementosPDF(page, sk, T, font) {
    ...
    (sk.cintas || []).forEach((c) => {
      ...
      const LYy = (tm, dd) => py(vay + c.uy * tm + c.inY * dd);
  ...
  function detalleCintasPDF(doc, datos, cibsa, font, bold, W, H, M) {
    ...
    const strip9 = (L9, seg9, tit9, off9, marks9) => {
      const altura = 76;   // v17-112: dos niveles alternados de etiquetas
      ...
      (seg9.opens || []).forEach((m) => {
        page.drawCircle({ x: bx((m.a + m.b) / 2), y: (bTop + bBot) / 2, size: 4.5, borderColor: EDGE, borderWidth: 0.8 });
      });
      // v17-112: etiquetas en DOS niveles alternados por posición — no se acoplan
      const feats9 = []
        .concat((seg9.opens || []).map((m) => ({ m: m, l1: (m.dia > 0 ? "O" + SK.fmt(m.dia) + " · " : "") + SK.fmt(off9 + m.a) + "-" + SK.fmt(off9 + m.b), l2: m.dia > 0 ? "loop" : "bolsillo", c1: NEG, c2: GRIS })))
        .concat((seg9.safety || []).map((m) => ({ m: m, l1: "! " + SK.fmt(off9 + m.a) + "-" + SK.fmt(off9 + m.b), l2: "c. seguridad", c1: REDE, c2: REDE })))
        .concat((seg9.gaps || []).map((m) => ({ m: m, l1: "x" + SK.fmt(m.b - m.a) + " · " + SK.fmt(off9 + m.a) + "-" + SK.fmt(off9 + m.b), l2: "hueco", c1: REDE, c2: GRIS })))
        .sort((p9, q9) => (p9.m.a + p9.m.b) - (q9.m.a + q9.m.b));
      feats9.forEach((ft9, i9) => {
        const tm9 = (ft9.m.a + ft9.m.b) / 2, niv9 = (i9 % 2) * 13;
        ctr9(page, ft9.l1, bx(tm9), bBot - 8 - niv9, 5.5, ft9.c1);
        ctr9(page, ft9.l2, bx(tm9), bBot - 14.5 - niv9, 5.5, ft9.c2);
      });
      (seg9.gaps || []).forEach((m) => {
      ...
      page.drawText(san(SK.fmt(off9)), { x: M, y: bBot - 36, size: 6, font: font, color: GRIS });
      { const lblF = san(SK.fmt(off9 + L9) + "m"); page.drawText(lblF, { x: M + availW - font.widthOfTextAtSize(lblF, 6), y: bBot - 36, size: 6, font: font, color: GRIS }); }
      (marks9 || []).forEach((mk) => {
